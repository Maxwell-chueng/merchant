...
export default {
  ...
  mutations: {
    isFromRegits(state){
      state.fromRegits = true;
    },
    clearFromRegits(state){
      state.fromRegits = false;
    },
    // 登陆后为用户设置相应的cookie
    Mlogin(state, params) {
  ...
  actions: {
    isFromRegits(ctx){
      ctx.commit("isFromRegits");
    },
    clearFromRegits(ctx){
      ctx.commit("clearFromRegits");
    },
    Alogin(ctx, params) {
